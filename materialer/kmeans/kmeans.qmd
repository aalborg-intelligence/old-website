...
$$ \|\vec p- \vec q\|^2 $$
er den kvadrerede afstand mellem to punkter repræsenteret ved stedvektorerne $\vec p$ og $\vec q$. Det vil sige, den [euklidiske afstand](../afstande/AfstandeMellemPunkteriPlanen.qmd#sec-euklidisk_afstand) mellem $\vec p$ og $\vec q$ opløftet i anden. Men her er vi kun interesseret i at se på kvadrerede afstande mellem punkter i samme gruppe. Derfor lægger vi kun kvadrerede afstand samme, hvis $\vec p$ og $\vec q$ er i samme gruppe -- alstå $\vec p\in S_i$ og $\vec q \in S_i$. Det er udtrykt i 

$$ \sum_{\vec p\in S_i}\sum_{\vec q\in S_i}\|\vec p-\vec q\|^2 $$

Nu kan der være nogle grupper med mange punkter og andre grupper med få punkter. For at tage højde for det dividerer vi ovenstående udtryk med antallet af punkter i gruppe $S_i$: $|S_i|$. Dette giver os en gennemsnitlig kvadreret afstand fra et punkt $\vec p$ til alle andre punkter $\vec q$ i samme gruppe som $\vec p$. Gør vi det, får vi
