---
title: "Sensitivitet og specificitet"
...
## Fejlklassifikationsraten

Når man skal vælge en god algoritme til at prædiktere klasser, har man brug for at kunne sammenligne, hvor godt forskellige algoritmer prædikterer. Man er derfor nødt til at have et mål for, hvor godt en algoritme prædikterer. Det mest oplagte er at tælle, hvor mange observationer algoritmen klassificerer forkert. Man kan så beregne, hvor stor en andel af alle observationerne, der klassificeres forkert. Denne andel kaldes *fejlklassifikationsraten*. Det er dog ikke altid det bedste mål at bruge. Det handler denne note om.
...
```{r , fig.height=3}
#| echo: false
#| fig-cap: Klassifikation med $t=10$. Observationer i det blå område prædikteres blå, mens observationer i det røde område  prædikteres røde. Udfyldte cirkler angiver korrekte klassifikationer. Åbne cirkler angiver fejlklassifikationer. 
#| label: fig-t10
x<-1:20
y<-c(rep(0,12),1,rep(0,2),1,0, 1 ,0,0 )
t<-10
...
|t|Antal fejl|Fejlklassifikationsrate|
|--|----|----|
|1|16|0.80|
|2|15|0.75|
|3|14|0.70|
...
: Fejlklassifikationer for forskellige værdier at $t$ {#tbl-fejlklassifikationer}

Den laveste fejlklassifikationsrate får vi ved at vælge $t=20$, sådan at alle observationer prædikteres til at være blå. Men hvis vi er ude på at identificere de sjældne røde, så er sådan en test jo ikke meget værd. I ovenstående eksempel kunne man i stedet have valgt at sætte $t=12$. Så får man ganske vist $5$ fejlklassifikationer i stedet for $3$. Til gengæld finder man alle de røde. Det virker som et mere fornuftigt valg i vores eksempel. 
...
Hvis vi udelukkende er interesserede i, hvor god algoritmen er til at prædiktere den røde klasse,  kan vi kigge på *sensitiviteten*. Dette er sandsynligheden for, at en observation, hvis sande farve er rød, faktisk bliver klassificeret som rød, altså 
$$\text{Sensitivitet} = P(\text{ En sand rød observation prædikteres som rød })$${#eq-sens}
...
$$\text{Specificitet} = P(\text{ En sand blå observation prædikteres som blå } )$$

Lad os se på eksemplet fra før, hvor vi sætter tærskelværdien til $t=15$. Vi udfylder confusion matricen med antallet af observationer i hver celle.
...
$$\text{Sensitivitet} = \frac{\text{Antal røde der prædikteres røde}}{\text{Antal røde}} = \frac{2}{3} = 0.667$${#eq-sens_formel}
Tilsvarende kan vi beregne specificiteten ved at se på anden søjle.
$$\text{Specificitet} = \frac{\text{Antal blå der prædikteres blå}}{\text{Antal blå}} = \frac{14}{3+14}= 0.824$$

Algoritmen er altså bedst til at finde blå, da specificiteten er højere end sensitiviteten.   Lad os prøve, om vi kan få højere sensitivitet med en anden værdi af $t$. Værdien $t=12$ var den mindste værdi, der kunne finde alle de røde. Det giver os confusion matricen i @fig-t12.
...
$$\text{Sensitivitet} = \frac{\text{Antal røde der prædikteres røde}}{\text{Antal røde}} = \frac{3}{3} = 1$$
og
$$\text{Specificitet} = \frac{\text{Antal blå der prædikteres blå}}{\text{Antal blå}} = \frac{12}{5+12}= 0.706$$
...
$$\text{Sensitivitet} = \frac{\text{Antal røde der prædikteres røde}}{\text{Antal røde}} = \frac{0}{3}= 0$$
og specificiteten
$$\text{Specificitet} = \frac{\text{Antal blå der prædikteres blå}}{\text{Antal blå}} = \frac{14}{17}= 1$$
...
$$P(A|B) = \frac{P(A\cap B)}{P(B)}$${#eq-betinget}
...
Lad os se på et eksempel, hvor vi slår to gange med en terning. Lad $A$ være hændelsen, at vi slår to seksere, og lad $B$ hændelsen, at den første terning viser seks. Da er $P(A)=P(\text{to seksere})=1/36$ og $P(B)=P(\text{første terning iser seks})=1/6$. Intuitivt vil man forvente, at sandsynligheden for to seksere vokser, hvis den første terning viser en sekser. Det kan vi bekræfte ved hjælp at betingede sandsynligheder.
$$P(\text{to seksere} | \text{første terning viser seks}) = P(A|B) =  \frac{P(A\cap B)}{P(B)} = \frac{1/36}{1/6} = \frac{1}{6}$$
Her udnyttede vi at $A\cap B=A$, da første terning er nødt til at vise seks for at vi kan få to seksere. Vi ser altså at
$$P(\text{to seksere} | \text{første terning viser seks}) = \frac{1}{6} \neq \frac{1}{36} = P(\text{to seksere})$$

Terningeksemplet viste et eksempel hvor $P(A)\neq P(A|B)$, altså hvor sandsynligheden for $A$ ændrer sig, hvis vi ved, at $B$ er indtruffet. Dette er ofte tilfældet. Nogle gange kan vi dog have at $P(A|B)=P(A)$, altså at vi ikke får nogen ny viden om sandsynligheden for $A$ ud fra vores viden om $B$. I dette tilfælde siger vi, at $A$ og $B$ er *uafhængige*.
...
\begin{align*}
\text{Sensitivitet} &= P(\text{ Prædiktionen er rød }|\text{ Den sande klasse er rød }) \\
&= \frac{P(\text{ Den sande klasse er rød og prædiktionen er rød })}{P(\text{ Den sande klasse er rød })}
\end{align*}
...
\begin{align*}\text{Sensitivitet} &= \frac{(\text{Antal røde der prædikteres røde})/(\text{Samlet antal})}{(\text{Antal røde})/(\text{Samlet antal})} \\
&= \frac{\text{Antal røde  der prædikteres røde}}{\text{Antal røde}}
\end{align*}
Det var netop den formel, vi brugte til at udregne sensitiviteten i (@eq-sens).

Specificiteten kan tilsvarende defineres som
\begin{align*}
\text{Specificitet} &= P(\text{ Prædiktionen er blå }|\text{ Den sande klasse er blå }) \\
&= \frac{P(\text{ Den sande klasse er blå og prædiktionen er blå })}{P(\text{ Den sande klasse er blå })}
\end{align*}
Man kan som ovenfor regne sig frem til, at specificiteten kan estimeres ved
\begin{align*}\text{Specificitet} = \frac{\text{Antal blå der prædikteres blå}}{\text{Antal blå}}
\end{align*}
...
Så hvordan vælger man en tærskelværdi, der giver en god afvejning mellem sensitivitet og specificitet? Som en hjælp kunne man udregne sensitivitet og specificitet for forskellige mulige værdier af $t$. Det er gjort i @tbl-sensitivitet.

|t|Sensitivitet| Specificitet |
|--|------|------|
|1|1|0.059|
|2|1|0.118|
|3|1|0.176|
...
Man kan så gå ind i @tbl-sensitivitet og lede efter et godt $t$, hvor både sensitivitet og specificitet er høj. En tabel som ovenfor bliver dog hurtigt uoverskuelig, hvis man har et stort datasæt. For at få overblik kan man i stedet vælge at tegne samhørende værdier af sensitivitet og specificitet ind i et koordinatsystem. Traditionelt vælger man at have $1-\text{specificitet}$ på $x$-aksen og $\text{sensitivitet}$ på $y$-aksen. Den kurve der fremkommer, når punkterne forbindes, kaldes en *ROC-kurve*[^2]. På @fig-roc er ROC-kurven fra dataeksemplet i @fig-data_eksempel indtegnet.
...
Vi vil gerne have både sensitivitet og specificitet til at være så tæt på $1$ som muligt. Vi søger derfor et punkt på ROC-kurven, der ligger så tæt på punktet $(0,1)$ som muligt. Ud fra ROC-kurven kunne punktet $(0.176,0.667)$ ligne et godt bud. Ifølge tabellen svarer det til en tærskelværdi på $t=15$.

På @fig-roc er identitetslinjen $y=x$ også indtegnet. Et punkt $(p,p)$ på identitetslinjen svarer til en helt tilfældig prædiktionsalgoritme, hvor hver observation bliver klassificeret som rød med sandsynlighed $p$ og blå med sandsynlighed $1-p$ uden at tage højde for værdien af $x$. For sådan en algoritme er sandsynligheden for at en sand rød prædikteres rød altså også $p$, så
$$\text{Sensitivitet} = P(\text{En sand rød prædikteres rød}) = P(\text{Rød prædiktion}) = p$$
Tilsvarende kan vi beregne specificiteten
$$\text{Specificitet} = P(\text{En sand blå prædikteres blå}) = P(\text{Blå prædiktion}) = 1- p$$
og derfor
$$1-\text{Specificitet} = 1-( 1- p) =  p$$
For sådan en test får vi altså
$$(1-\text{specificitet},\text{sensitivitet}) = (p,p) $$
Alt i alt viser dette, at punkterne på identitetslinjen svarer til helt tilfældig prædiktion. En prædiktionsalgoritme skal derfor helst give et punkt, der ligger over identitetslinjen. Ellers er den ikke bedre end et tilfældigt gæt.
...
Husk på, at ROC-kurven gerne skulle ligge så tæt op mod punktet  $(0,1)$ og så langt over diagonalen som muligt. Vi kan derfor bruge *arealet under ROC-kurven*, også kaldet *AUC*[^11], som mål for hvor meget ROC-kurven er strakt opad mod $(0,1)$. 
...
Ofte har man i praksis mere end én inputvariabel $x$ at prædiktere ud fra. Lad os sige, at vi har målt variablene $x_1,x_2,\ldots,x_p$. Mange algoritmer (blandt andet perceptronen, sigmoid neuroner, kunstige neurale netværk og logistisk regression **links, flere?**) laver prædiktionerne ud fra en vægtet sum af variablene
$$w_1x_1 + w_2x_2 + \dotsm +w_p x_p$$
hvor $w_1, w_2,\ldots,w_p \in \mathbb{R}$ er konstanter.
Man prædikterer så den ene klasse når 
$$w_1x_1 + w_2x_2 + \dotsm +w_p x_p > t$$ 
og den anden klasse når
$$w_1x_1 + w_2x_2 + \dotsm +w_p x_p \leq t$$ 
...
I eksemplet fra @fig-data_eksempel fandt vi, at $t=15$ virkede som et fornuftigt valg. Det var i hvert fald et $t$, der passede godt på det datasæt, vi havde. Det betyder dog ikke, at det er det $t$, der generaliserer bedst til nyt data. Lad os sige, at vi får et nyt datasæt og gerne vil bruge prædiktionsalgoritmen på det. På @fig-test-data ses et eksempel på, hvordan et nyt datasæt kunne se ud. 
```{r }
#| echo: false
#| fig-cap: Det oprindelige data fra @fig-data_eksempel og et nyt datasæt klassificeret ud fra $t=15$
#| label: fig-test-data
par(mfrow=c(2,1))
...
$$
\text{Positiv prædiktiv værdi} = P(\text{ En rød prædiktion er faktisk rød } ) 
$$
I praksis beregnes den positive prædiktive værdi ved formlen 
$$
\text{Positiv prædiktiv værdi} = \frac{\text{Antal røde prædiktioner som faktisk er røde}}{\text{Antal røde prædiktioner}}
$$
...
$$
\text{Negativ prædiktiv værdi} = P(\text{ En blå prædiktion er faktisk blå } ) 
$$
Den negative prædiktive værdi kan beregnes ved
$$
\text{Negativ prædiktiv værdi} = \frac{\text{Antal blå prædiktioner som faktisk er blå}}{\text{Antal blå prædiktioner}}
$$
...
\begin{align*}
\text{Positiv prædiktiv værdi} &= P(\text{ Den sande klasse er rød }|\text{ Prædiktionen er rød }) 
\end{align*}
...
Vi forlænger bøken med $P(A)$ og bruger en brøkregneregel 
...
Lader vi $A=\{\text{Sand rød}\}$ og $B=\{\text{Rød prædiktion}\}$ i Bayes' formel, får vi følgende sammenhæng mellem positiv prædiktiv værdi og sensitivitet 
$$
\text{Positiv prædiktiv værdi} = P(\text{ Sand rød }| \text{ Rød prædiktion })$$
$$= P( \text{ Rød prædiktion }| \text{ Sand rød })\cdot \frac{P(\text{ Sand rød })}{P(\text{ Rød prædiktion })}$$
$$= \text{Sensitivitet}\cdot \frac{P(\text{ Sand rød })}{P(\text{ Rød prædiktion })}
$${#eq-ppv}
...
Lad os se lidt nærmere på nævneren i (@eq-ppv), det vil sige $P(B)=P(\text{Rød prædiktion})$. Husk på at $A=\{\text{Sand rød}\}$. *Komplementærhændelsen* til $A$ er hændelsen at $A$ ikke indtræffer, og betegnes $A^c$. I vores tilfælde er  $A^c=\{\text{Ikke sand rød}\}=\{\text{Sand blå}\}$. Hændelsen $B$ at prædiktionen er rød kan opnås ved, at prædiktionen er rød, og den underliggende klasse er rød, svarende til $B \cap A$, eller ved at prædiktionen er rød, og den sande klasse er blå, svarende til $B\cap A^c$. Vi kan derfor beregne sandsynligheden for $B$ som summen
...
$$\text{Positiv prædiktiv værdi} = \text{Sensitivitet}\cdot \frac{P(\text{Sand rød})}{P(\text{Rød prædiktion})}$$
$$= \text{Sensitivitet}\cdot \frac{P(\text{Sand rød})}{P(\text{Rød prædiktion}|\text{Sand rød})P(\text{Sand rød}) + P(\text{Rød prædiktion}|\text{Sand blå})P(\text{Sand blå}) }$$
$$= \text{Sensitivitet}\cdot \frac{P(\text{Sand rød})}{\text{Sensitivitet}\cdot P(\text{Sand rød}) + (1-\text{Specificitet})\cdot P(\text{Sand blå}) }
$${#eq-ppv-formel}
Vi ser, at hvis specificiteten ikke er meget høj, og sandsynligheden for sand rød er lav (og dermed sandsynligheden for sand blå høj), så er tælleren i (@eq-ppv-formel) lille i forhold til nævneren. Den positive prædiktive værdi vil derfor være væsentligt lavere end specificiteten. Det var det, der skete i eksemplet (@eq-ppv-eks).

Formlen (@eq-ppv-formel) viser desuden, at den positive prædiktive værdi afhænger af ikke baresensitivitet og specificitet, men også af fordelingen mellem de to klasser (altså sandsynligheden for sand rød og sand blå). Hvis fordelingen ændrer sig, så den røde klasse for eksempel bliver mere sandsynlig, så ændrer den positive prædiktive værdi sig også.
