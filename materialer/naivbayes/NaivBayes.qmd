...
Udtrykket $P(A \cap B)$ i tælleren er sandsynligheden for *fælleshændelsen* mellem $A$ og $B$ -- det vil sige hændelsen at både $A$ og $B$ omtræffer -- og i nævneren sørger vi for, at man kun ser på de udfald, hvor $B$ er givet[^1].
...
\end{aligned}
$$
Her vi dog nødt til at antage, at $$P(Y = 1 \mid \mathbf{X} = \mathbf{x})\neq 0,$$ så vi ikke kommer til at dividere med $0$. 
...
Vi kan sammenfatte udtrykket i (@eq-forhold_posterior_ssh) det således til det såkaldte *posterior forhold*:
$$
...
men blot for gange i stedet for addition.

## Posterior forholdet, score og vægte

Samler vi nu udtrykkene, som indgår i vores posterior forhold i (@eq-forhold_posterior_ssh), samtidig med at vi antager, at $X_1, X_2, \cdots, X_q$ er uafhængige af hinanden giver $Y$, får vi nedenstående:
...
Vi minder om, at forholdet mellem $P(Y = 0 \mid \mathbf{X} = \mathbf{x})$ og $P(Y = 1 \mid \mathbf{X} = \mathbf{x})$ var af særlig interesse omkring værdien 1 -- se (@eq-binar_bayes_klassifikation2). Når forholdet er $1$ betyder det, at de to klasser er lige sandsynlige givet $\mathbf{x}$. Endvidere, når forholdet var er $1$, er $Y=0$ mere sandsynlig end $Y=1$, og når det er under $1$, er det omvendte tilfældet.
...
$$
kan vi således skrive udtrykket for $S$ i (@eq-posterior_forhold_ln) som 
$$
...
Alle tilsvarende sandsynligheder kan beregnes, så man kan se hvor stor en andel af stemmerne på de to blokke, der komme fra hver gruppe. Resultatet ses i nedenstående tabel:

|     | Mænd   | Kvinder | Ung    | Ældre  | Sjælland | Jylland | Anden bopæl  |
|-----|--------|---------|--------|--------|----------|---------|--------|
