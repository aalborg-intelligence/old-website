...
I noten om [simple neurale netværk](simple_neurale_net.qmd) beskrev vi, hvordan vi kan tænke på værdi 

$$
\sigma(w_0 + w_1 \cdot x_1 + \cdots + w_n \cdot x_n)
$$
...
\textrm{Nyt punkt }=
\begin{cases}
\textrm{blåt} & \textrm{hvis } \sigma(w_0 + w_1 \cdot x_1 + \cdots + w_n \cdot x_n) \geq 0.5\\
\textrm{rødt} & \textrm{hvis } \sigma(w_0 + w_1 \cdot x_1 + \cdots + w_n \cdot x_n) < 0.5\\
\end{cases}
$$ {#eq-pred}
...
Der er her fin overensstemmelse mellem targetværdien og den beregnede sandsynlighed. Læg også mærke til at det ekstreme punkt har en beregnet sandsynlighed på $0.00$ og dermed bliver prædikteret til klart at være et *ikke* blåt -- det vil sige et rødt -- punkt.
