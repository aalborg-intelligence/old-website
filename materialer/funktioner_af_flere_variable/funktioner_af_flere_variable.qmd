...
description: En funktion kan godt afhænge af flere forskellige variable, og i det tilfælde taler man om **funktioner af flere variable**. Denne type af funktioner viser sig at spille en helt central rolle i rigtig mange metoder inden for kunstig intelligens. Derfor vil vi i denne note behandle de vigtigste begreber i forbindelse med funktioner af flere variable.
from: markdown+emoji
format:
    html:
      self-contained: true 
...
[^1]: Værdimængden for funktionen behøver ikke at være hele $\mathbb{R}$, men kan godt bare være en delmængde af $\mathbb{R}$.
...
Vi vil nu for en funktion af to variable tage udgangspunkt i et konkret punkt $(x_0, y_0, f(x_0,y_0))$.  Forestil dig at vi laver et lodret snit parallel med $x$-aksen ned gennem grafen for $f$. Det er lidt ligesom, at lave et snit ned gennem en lagkage! Et eksempel er vist i @fig-graf_snitkurve_y. Den lyseblå plan svarer til den lagkagekniv, vi har skåret med, og den skærer altså lodret og er samtidig parallel med $x$-aksen.

Når man laver sådan et lodret snit med grafen for $f$, så kalder man skæring mellem den lodrette plan (lagkagekniven) og grafen for $f$ for en *snitkurve*. Snitkurven er markeret med sort på @fig-graf_snitkurve_y. Den funktion, hvis graf svarer til snitkurven, kaldes for en *snitfunktion*. Forskriften for snitfunktionen fås ved, at vi fastholder $y$-værdien på  $y_0$ og lader $x$ varierer:

$$
g(x)=f(x,y_0).
$$
Vi kan også lave et lodret snit gennem $(x_0,y_0,f(x_0,y_0))$, men hvor snittet i stedet er parallel med $y$-aksen -- se @fig-graf_snitkurve_x. Det svarer til, at vi fastholder $x$ på værdien $x_0$ og lader $y$ variere. Gør vi det fås snitfunktionen
...
Som illustreret i @fig-graf_snitkurve_partialx har grafen for snitfunktionen $g(x)=f(x,y_0)$ en tangent i punktet $P(x_0, y_0, f(x_0,y_0))$ med hældning $f_x'(x_0,y_0)$. Det betyder grafisk, at hvis vi står i punktet $P$ og gerne vil bevæge os langs tangentens, så skal vi: Bevæge os $1$ enheden i $x$-aksens retning, $0$ enheder i $y$-aksens retning (husk på at snitkurven forløber i planen med ligning $y=y_0$, hvor $y$ er fastholdt, og dermed ikke ændrer sig) og $f_x'(x_0,y_0)$ enheder i $z$-aksens retning. Dermed vil en retningsvektor for denne tangent være
...
På helt tilsvarende vis vil grafen for snitfunktionen $h(y)=f(x_0,y)$ have en tangent i punktet $P(x_0, y_0, f(x_0,y_0))$ med hældning $f_y'(x_0,y_0)$ -- se @fig-graf_snitkurve_partialy. Derfor vil en retningsvektor for denne tangent være
...
Tegner vi en repræsentant for denne vektor i $xy$-planen med udgangspunkt i punktet $Q(1,-2,0)$ (som er projektionen af $P(1,-2,-7)$ op på $xy$-planen) ses resultatet i @fig-graf_gradient. Gradienten angiver altså en retning i $xy$-planen, og laver man et lodret snit i denne retning fås igen en snitkurve, som også ses indtegnet med sort på @fig-graf_gradient.
