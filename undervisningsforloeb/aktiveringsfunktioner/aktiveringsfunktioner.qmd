...
I opbygningen af kunstige neurale netværk er aktiveringsfunktioner helt centrale. Hvis ikke man bruger aktiveringsfunktioner i et kunstigt neuralt netværk, vil man faktisk bare bygge en stor lineær funktion af input-værdierne. Og lineære funktioner kan man ikke prædiktere ret meget med!
...
Når man træner en AI model, sker det som regel ved, at man forsøger at minimere en såkaldt tabsfunktion $E$. En **tabsfunktion** har til formål at \"måle\", hvor god en AI model[^1] er. En tabsfunktion $E$ har altid den egenskab, at $E \geq 0$, og at en lille værdi af $E$ svarer til en god model (der er et lille tab), mens en stor værdi af $E$ svarer til en mindre god model. Derfor vælger man den model, som giver den mindste værdi af tabsfunktionen. 
...
Når tabsfunktionen skal minimeres bruger man det, man kalder for **træningsdata**. Det betyder, at man har givet en række **inputværdier** $x_1, x_2, \dots, x_n$ på baggrund af hvilke, man ønsker at prædiktere en såkaldt **targetværdi** $t$. 
...
[^2]: Når en perceptron trænes ved hjælp af Adaline, er dette faktisk ikke tilfældet. Her er targetværdien $t \in \{-1,1\}$, mens outputværdien $o \in \mathbb{R}$. Det giver faktisk nogle problemer, som er beskrevet i noten om [simple neurale netværk](../simple_neurale_net/simple_neurale_net.qmd).
...
![](images/exp_minusx.png){width=50% fig-align="center"}
...
![Grafen for $|x|$.](images/numerisk_x.png){width=75% #fig-numerisk_x}
...
![Grafen for hyperbolsk tangens.](images/tanh.png){width=75% #fig-tanh}
...
![](images/exp_plusminusx.png){width=50% fig-align="center"}
...
|:------:|:------:|:------:|:------:|:------:|:------:|
| Sigmoid | $\frac{1}{1+e^{-x}}$ | ![](images/sigmoid.png) | $(0,1)$ | $f(x)\cdot(1-f(x))$ |
| Softsign | $\frac{x}{1+|x|}$ | ![](images/softsign.png) | $(-1,1)$ | $(1-|f(x)|)^2$ | 
| Hyperbolsk tangens | $\frac{e^x-e^{-x}}{e^x+e^{-x}}$ | ![](images/tanh.png) | $(-1,1)$ | $1-\left ( \tanh(x) \right )^2$| 
| ReLU | $\begin{cases}
x & \textrm{hvis } x > 0 \\ 
0 & \textrm{hvis } x \leq 0 \\ 
\end{cases}$ | ![](images/ReLU.png) | $[0,\infty)$ | $\begin{cases}
1 & \textrm{hvis } x > 0 \\ 
0 & \textrm{hvis } x \leq 0 \\ 
\end{cases}$| 
