...
Find selv på forskellige værdier af $b$ og $c$ og find ud af om det tilhørende andengradspolynomium har rødder eller ej. 
Du skal finde på mindst to der har rødder og to der ikke har, men gerne et par stykker mere.
Skriv dine værdier ned (enten bare på papir eller i f.eks. et regneark) og indtegn dem i et koordinatsystem hvor værdien af $b$ er på 1. aksen og værdien af $c$ er på 2. aksen. 
Herunder er lavet et eksempel med $b=0$ og $c=-1$ som har rødder samt $b=2$ og $c=4$ som ingen rødder har.
```{r echo=FALSE}
library(ggplot2)
...
Perceptronen[^2] gør dybest set det, at den prøver at bestemme en ret linje, som kan bruges til at adskille de røde punkter fra de blå punkter i punktplottet ovenfor. 
...
Hvilken linje svarer det til? Indtegn den i koordinatsystemet. Adskiller denne linje de to grupper af punkter (med og uden rødder)? Hvis grupperne allerede er adskilt skal du tilføje punktet med $b=2$ og $c=1$, som svarer til et 2. gradspolynomium der har en rod.
...
w_0 + w_1 \cdot b + w_2 \cdot c > 0
$$
og får outputværdien $o=1$ mens dem der ligger under opfylder den omvendte ulighed og får outputværdien $o=-1$.

Udvælg et punkt der bliver fejlklassificeret. Dvs. som enten ligger under linjen ($o=-1$), men har target $t=1$ svarende til ingen rødder eller omvendt. Udregn fejlen $error=t-o$ som enten er -2 eller 2.
...
- Fik du en linje der adskiller de to grupper?
...
::: {.callout-note collapse="false" appearance="minimal"}
Lav et nyt koordinatsystem og indtegn dine træningsdata med værdien af $b^2$ på 1. aksen og værdien af $c$ på 2. aksen. Hvilken linje kan du vælge til at adskille de to grupper?
:::
